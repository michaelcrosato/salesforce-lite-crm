...
1a. (Item 54 backend) Create `lib/featureFlags.ts`:
    - Export a const object `FEATURE_FLAGS` with boolean entries:
      ...
      `areaEdit: false`.
    - Export a typed helper `isEnabled(flag: keyof typeof FEATURE_FLAGS): boolean`.
    - Export `EXCLUDED_ROUTES: readonly string[]` listing the routes
      that should 404 or show a placeholder: `/tasks`, `/cases`,
      `/campaigns`, `/deals/[id]`, etc. (use the canonical Next.js
      path format).
    - Document in CRM-CONTRACT.md a new section "Feature flags and
...
1b. (Item 56 backend) Create `lib/postal.ts`:
    - `normalizePostalCode(input: string, country: 'CA' | 'US'): string | null`.
      ...
      `^\d{5}(-\d{4})?$`. Return null on invalid.
    - `extractPostalPrefix(normalized: string, country: 'CA' | 'US'): string`.
      For CA: first 3 chars (FSA). For US: first 5 chars (ZIP).
    - `validatePostalCode(input: string, country: 'CA' | 'US'): { ok: true; normalized: string; prefix: string } | { ok: false; reason: string }`.
      ...
      Claude will display these verbatim.
    - Export Zod schema `postalCodeSchema` for use in lead form
...
1c. (Item 55 backend) Extend `lib/services/leads.ts` (or wherever
    ...
    - Implementation reads the `routing_event` Activity tied to
      ...
      record.
    - Update `lib/crm/crmClient.ts` to expose this as
      `crmClient.leads.getRoutingDecision(id)`.
    - Document the new method + return type in CRM-CONTRACT.md
