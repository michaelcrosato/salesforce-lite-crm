...
const mockOrders: TrophyOrder[] = [
  { id: "d1", name: "Vancouver Northstar", monthlyQuota: 28, deliveredThisMonth: 31, account: { id: "a1", name: "Northstar Freight" } },
  { id: "d2", name: "Calgary Luma", monthlyQuota: 16, deliveredThisMonth: 9, account: { id: "a2", name: "Luma Health" } },
  { id: "d3", name: "Toronto Orbit", monthlyQuota: 24, deliveredThisMonth: 27, account: { id: "a3", name: "Orbit Retail" } },
  { id: "d4", name: "Edmonton Apex", monthlyQuota: 15, deliveredThisMonth: 4, account: { id: "a4", name: "Apex Cloud" } },
];
...
describe("YOLO Dealer Trophies & Mascots", () => {
  it("awards at least one ridiculous trophy when someone is over quota", () => {
    ...
    expect(trophies.some((t) => t.emoji === "💥" || t.emoji === "🐆" || t.emoji === "🦙")).toBe(true);
  });
  ...
  it("gives every dealer a personal ridiculous mascot", () => {
    ...
    const validMascots = ["Turbo Llama", "Pacing Panther", "Golden Shovel", "Quota Crusher", "Disco Dealer", "Lead Eagle", "Maple Moose", "Rage Router"];
    expect(validMascots).toContain(mascot.name);
  });

  it("detects most improved pacing between two periods", () => {
    const previous: TrophyOrder[] = mockOrders.map((o) => ({ ...o, deliveredThisMonth: Math.floor(o.deliveredThisMonth * 0.6) }));
