# AI Operations Plan — salesforce-lite-crm
...
| Code host & gate | **GitHub** (`michaelcrosato/salesforce-lite-crm`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **Local (no cloud deployment)** — QA runs against `localhost:3000` seeded with `npm run seed`; the charter permanently defers deployment configuration. Preview links in PRs link to local screenshots/evidence until a deployment surface is promoted. | Single-tenant local-first CRM; charter non-goal: deployment config. |
| Database | **SQLite via Prisma** (local file `prisma/dev.db`) — single dev database; `npx prisma db push` applies schema; migrations via Prisma; no production database until explicitly promoted | Local-first single-tenant charter; Postgres schema exists at `prisma/schema.postgres.prisma` but is not the default runtime. |
| E2E verification | **Playwright** driven by agents ("test like a human user") against seeded data (`npm run seed && playwright test`) | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
- **Allow:** `npm` scripts, `node`, `tsc`, `biome`, `vitest`, `prisma` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
