...
any conflict. `AGENTS.md` is the operating handoff. `docs/ai/REPO_MAP.md` is
the navigation map.

## Purpose

Salesforce Lite CRM is a local-first Next.js + Prisma CRM for small-business
revenue ops (accounts, contacts, opportunities, activities, tasks, cases,
campaigns, consumer-lead → dealer-order routing, forecasting, reports, CSV review,
command-palette search). It is designed so autonomous coding agents can orient and
extend safely within a shared contract and deterministic local gate.

## Current state

- AFK readiness baseline is in place for this run: `scripts/agent/*.sh` has
  hardened package-manager detection and skip/no-op reporting, and core
  documentation has been aligned for reviewability.
- Local gate status for this branch: `lint`, `typecheck`, `test` (562 tests),
  and `build` are re-verified green on 2026-05-29; `test:e2e` is unchanged since
  2026-05-28 (`50/50`).
...
minutes, run one gate command for pass/fail truth, and extend within the contract.
Docs describe what *exists*, not aspiration.
...
- `prisma/schema.prisma` and `prisma/seed.ts` are sacred (`CLAUDE.md` [tags and
  schema-changelog requirements] and `PLAN.md` shared ownership rules).
- `.claude/**` hook config is sacred.
- No `any` / `@ts-ignore`; enforced by lint + typecheck.

## Agent guidance

**Read first:** `PLAN.md` §1–4 → `CRM-CONTRACT.md` → `AGENTS.md` → `GOAL.md` →
...
commits. **Avoid:** product scope expansion, broad refactors during feature work,
claiming unrun checks, bypassing `PLAN.md` guardrails.

## Definition of done

`lint`/`typecheck`/`test`/`build` attempted with real results recorded; docs
match actual behavior; changes are atomic and scoped; findings outside scope are
filed as tickets, not half-implemented.
