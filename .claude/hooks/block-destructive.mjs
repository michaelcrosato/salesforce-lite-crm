...
const patterns = [
  {
    re: /\brm\s+(-[a-zA-Z]*[rRf][a-zA-Z]*\s+)?\/(?!tmp\b|var\/folders\b)/,
    why: "rm against root or system path"
  },
  { re: /\brm\s+(-[a-zA-Z]*[rRf][a-zA-Z]*\s+)?[*?]/, why: "rm with wildcard" },
  {
    re: /\brm\s+-[a-zA-Z]*[rRf][a-zA-Z]*\s+(node_modules|\.git|\.next|dist|build|prisma)\b/,
    why: "rm against a critical directory"
  },
  ...
  { re: /\bgit\s+clean\s+-[fdx]+/, why: "git clean (data loss risk)" },
  {
    ...
    why: "git checkout -- . (discards changes)"
  },
  {
    re: /\bprisma\s+migrate\s+reset\b/,
    why: "prisma migrate reset (destroys data)"
  },
  {
    re: /\bprisma\s+db\s+push\s+[^|;&]*--force-reset\b/,
    why: "prisma force reset"
  },
  ...
  {
    re: /\bcurl\b.*\|\s*(sudo\s+)?(bash|sh|zsh)\b/,
    why: "pipe curl into shell"
  },
...
for (const { re, why } of patterns) {
  if (re.test(cmd)) {
    ...
    emitDeny(
      `Blocked destructive command (${why}). If genuinely required, add [DESTRUCTIVE OK <reason>] to the prompt referencing a PLAN.md entry, or use an approved cleanup script.`
    );
  }
}
