...
let changed = [];
try {
  changed = execSync("git status --porcelain", { cwd })
    .toString()
    .split("\n")
...
const scopeChecks = [
  {
    re: /^app\/deals\/\[id\]\//,
    why: "/deals/[id] route is out of scope (current spec: drawer at /deals?deal=<id>)"
  },
  {
    re: /\b(openai|anthropic-sdk|@google\/generative-ai|gemini-pro|claude-3|gpt-4|grok-api)\b/i,
...
for (const f of changed) {
  for (const c of scopeChecks) {
    if (c.re.test(f)) {
      ...
      emitBlock(
        `Cannot stop - out-of-scope change (${c.why}): ${f}. Remove the change OR promote it into PLAN.md.`
      );
    }
  }
}
