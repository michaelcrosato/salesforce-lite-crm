...
test.describe("hardened demo path", () => {
  test("intake -> routing -> analyst -> contact note -> forecast", async ({
    page
  }, testInfo) => {
    ...
    await expect(
      routingDetail.getByTestId("routing-detail-success")
    ).toBeVisible();

    await leadRow.getByTestId("routing-detail-link").click();
    await expect(leadRow.getByTestId("lead-status-badge")).toHaveText(
      "Assigned"
    );
    await expect(
      leadRow.getByTestId("lead-assignment-reason-badge")
    ).toHaveText("Routed");
    ...
    await expect(
      page.getByTestId("analyst-item-behind-pace-order").first()
    ).toContainText("Vancouver");
    await expect(
      page.getByTestId("analyst-item-stale-high-value-deal").first()
    ).toBeVisible();
    await expect(
      page.getByTestId("analyst-item-low-health-account").first()
    ).toBeVisible();
    ...
    await page
      .getByTestId("contact-note-input")
      .fill("Interested in fleet package. Quote by Friday.");
    await page.getByTestId("contact-note-submit").click();
    await expect(
      page.getByTestId("activity-timeline-summary").first()
    ).toContainText("Interested in fleet package");
    ...
    await expect(
      page.getByTestId("forecast-projection-value").first()
    ).not.toHaveText("$0.00");
  });
});
