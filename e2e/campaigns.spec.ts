...
test("create campaign, edit dates, and verify in list", async ({ page }) => {
  ...
  await page.getByLabel("Description").fill("Outreach campaign created by e2e spec.");
  ...
  await expect(page.getByText("Campaign updated.", { exact: true })).toBeVisible();

  await page
    .getByLabel(`Move ${campaignName} status`)
    .selectOption("active");
  await expect(page.getByText("Campaign status updated.", { exact: true })).toBeVisible();
