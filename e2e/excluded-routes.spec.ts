...
test.describe("excluded-route guard rails", () => {
  ...
  for (const route of excludedRoutes) {
    test(`route ${route.path} is excluded (returns 404 or placeholder)`, async ({
      page
    }) => {
      ...
      if (response?.status() === 200) {
        ...
        await expect(placeholder.or(heading)).toBeVisible();
      } else {
  ...
  test("command palette / global search is not implemented", async ({
    page
  }) => {
