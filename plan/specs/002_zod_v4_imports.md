...
- [ ] Every source `import … from "zod"` becomes `from "zod/v4"` (prefer `import * as z from "zod/v4"`).
- [ ] Error-handling code reconciled to the v4 issue API (v4 uses `error.issues`; verify any `.flatten()` / `.format()` / `.errors` usages still behave — fix call sites if shape differs).
- [ ] `npm run typecheck`, `npm run test` (565 tests), `npm run build` all green — **zero behavior change**.
- [ ] `grep -rn "from \"zod\"" --include=*.ts` returns no bare-`zod` imports in source (test fixtures excluded only if deliberate).
