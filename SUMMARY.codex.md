...
- `scripts/check-worktrees.ps1` shows Claude with untracked `tsconfig.tsbuildinfo` and Gemini with dirty `next-env.d.ts` plus untracked `tests/api/leadActions.test.ts`; those are other-agent worktree cleanup/accounting concerns, not Codex blockers.
