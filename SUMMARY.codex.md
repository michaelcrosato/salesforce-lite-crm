...
- Verified the baseline and final state with `powershell -ExecutionPolicy Bypass -File scripts/local-gate.ps1`: `npm install`, Prisma generate/db push, seed, 143/143 Vitest tests, build, Playwright Chromium install, and 19/19 e2e tests all passed.
...
- Hardened the shared list-query helper so non-finite direct-call pagination falls back to safe page and page-size defaults instead of producing `NaN` skip/take clauses; added regression coverage in `tests/api/listQuery.test.ts`.
