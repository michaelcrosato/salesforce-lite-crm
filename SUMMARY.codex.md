...
- Verified the baseline and final state with `powershell -ExecutionPolicy Bypass -File scripts/local-gate.ps1`: `npm install`, Prisma generate/db push, seed, lint, typecheck, 152/152 Vitest tests, build, Playwright Chromium install, and 19/19 e2e tests all passed.
...
- Added shared query-param sanitizers and used them on task, case, campaign, and lead list pages so blank or invalid filter params do not leak into service filters or stale form defaults; added focused helper coverage.
- Updated the forecast page to parse multiplier, assignment rate, and area query params independently so one malformed scenario field does not discard otherwise valid filters; expanded query-param helper coverage.
- Hardened shared date formatters so malformed date-like values fall back to existing empty-state labels instead of throwing or rendering `NaN days ago`; added focused formatter coverage.
- Hardened stale-deal and dashboard focus date scoring so malformed activity dates cannot create implicit `NaN` ranking behavior; added focused business regressions.
- Sanitized forecast order quantity inputs so malformed delivered/quota values cannot produce `NaN` projections; added a forecast regression.
