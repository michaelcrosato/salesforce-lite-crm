...
- Parsed forecast scenario query fields independently so malformed numeric
...
- Sanitized forecast order quantities so malformed delivered/quota values do
  not produce `NaN` projections.
